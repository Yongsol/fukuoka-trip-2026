import { events, places, restaurantGuide, defaultChecklist, dateLabels, transportGuides, officialLinks } from './data.js?v=22';
...
function renderFood(){
  document.querySelector('#food-map-legend').innerHTML=restaurantGuide.map(p=>`<button type="button" data-focus-restaurant="${esc(p.id)}"><i>${p.number}</i><span>${esc(p.koreanName)}</span></button>`).join('');
  document.querySelector('#food-list').innerHTML=restaurantGuide.map(p=>`<article class="food-card restaurant-card" id="restaurant-${esc(p.id)}" data-restaurant="${esc(p.id)}" tabindex="0" aria-labelledby="restaurant-title-${esc(p.id)}"><div class="restaurant-photo-wrap"><img class="restaurant-photo" src="${esc(p.image)}" alt="${esc(p.imageAlt)}" loading="lazy" width="640" height="420"><button class="restaurant-number" type="button" data-focus-restaurant="${esc(p.id)}" aria-label="지도에서 ${p.number}번 ${esc(p.koreanName)} 보기">${p.number}</button><a class="photo-source" href="${esc(p.imageSource)}" target="_blank" rel="noopener">사진 출처 ↗</a></div><div class="restaurant-copy"><div class="restaurant-meta"><span>${esc(p.category)}</span><span>${esc(p.area)}</span></div><h3 id="restaurant-title-${esc(p.id)}">${esc(p.koreanName)}</h3><p class="japanese-name" lang="ja">${esc(p.name)}</p><p class="hotel-access">🏨 ${esc(p.hotelAccess)}</p><ul class="menu-list">${p.menus.map(menu=>`<li><span>${esc(menu.name)}</span><strong>${esc(menu.price)}</strong><small>${esc(menu.priceNote)}</small></li>`).join('')}</ul><address>📍 ${esc(p.address)}</address><p class="restaurant-evidence"><b>근거</b> ${esc(p.evidenceNote)}</p><div class="card-links">${restaurantCardLinks(p)}</div></div></article>`).join('');
}
