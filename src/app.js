...
function renderFoodMapMarkers(){
  ...
  const mapRegion=document.querySelector('#food-map');if(mapRegion)mapRegion.setAttribute('aria-label',foodScope==='attached'?'첨부 맛집 11곳의 번호 위치 지도':'전체 맛집 25곳의 번호 위치 지도');
  const status=document.querySelector('#food-map-status');if(status)status.textContent=foodScope==='attached'?'첨부 인포그래픽의 1–11번을 표시했습니다. 번호를 누르면 같은 카드로 이동해요.':'저장한 맛집 1–25번을 모두 표시했습니다. 밀집 구역은 확대해 보세요.';
}
