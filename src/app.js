...
let state=loadState(); let selectedDate='2026-08-28'; let selectedRouteDate='2026-08-28'; let map; let foodMap; let foodMarkers=new Map(); let markers=[]; let routeLayer; let overviewRouteLayer; let overviewStopMarkers=[]; let transportMarker; let overviewRouteRequest; let nominatimRequest=null; let lastNominatimStarted=0;
...
function renderSocialFood(){
  ...
  list.innerHTML=socialFoodCandidates.map(candidate=>{
    ...
    const duplicate=numbers.length?`<p class="social-food-duplicate">기존 #${numbers.join('·#')} 카드와 같은 후보</p>`:'';
    const badge=candidate.verified?'사진에서 상호 판독':'사진 표기 · 정확한 지점 확인 필요';
    return `<article class="social-food-card"><div class="social-food-photo"><img src="${esc(candidate.image)}" alt="${esc(candidate.title)} SNS 저장 사진" loading="lazy" width="527" height="695"></div><div class="social-food-copy"><span class="social-food-badge ${candidate.verified?'is-read':'needs-check'}">${badge}</span><h3>${esc(candidate.title)}</h3><p>${esc(candidate.subtitle)}</p>${duplicate}<a class="social-map-link" href="${googleMapsUrl(candidate)}" target="_blank" rel="noopener">Google Maps 검색 ↗</a><small>사용자 제공 SNS 캡처 · 원 게시물/이용 권한 확인 권장</small></div></article>`;
  }).join('');
}
function renderFood(){document.querySelector('#food-map-legend').innerHTML=restaurantGuide.map(p=>`<button type="button" data-focus-restaurant="${esc(p.id)}"><i>${p.number}</i><span>${esc(p.koreanName)}</span></button>`).join('');document.querySelector('#food-list').innerHTML=restaurantGuide.map(p=>`<article class="food-card restaurant-card" id="restaurant-${esc(p.id)}" data-restaurant="${esc(p.id)}" tabindex="0" aria-labelledby="restaurant-title-${esc(p.id)}"><div class="restaurant-photo-wrap"><img class="restaurant-photo" src="${esc(p.image)}" alt="${esc(p.imageAlt)}" loading="lazy" width="640" height="420"><button class="restaurant-number" type="button" data-focus-restaurant="${esc(p.id)}" aria-label="지도에서 ${p.number}번 ${esc(p.koreanName)} 보기">${p.number}</button><a class="photo-source" href="${esc(p.imageSource)}" target="_blank" rel="noopener">사진 출처 ↗</a></div><div class="restaurant-copy"><div class="restaurant-meta"><span>${esc(p.category)}</span><span>${esc(p.area)}</span></div><h3 id="restaurant-title-${esc(p.id)}">${esc(p.koreanName)}</h3><p class="japanese-name" lang="ja">${esc(p.name)}</p><ul class="menu-list">${p.menus.map(menu=>`<li><span>${esc(menu.name)}</span><strong>${esc(menu.price)}</strong><small>${esc(menu.priceNote)}</small></li>`).join('')}</ul><address>${esc(p.address)}</address><div class="card-links"><a href="${googleMapsUrl(p)}" target="_blank" rel="noopener">Google Maps</a><a href="${esc(p.officialUrl)}" target="_blank" rel="noopener">공식 정보</a><a href="${esc(p.menuUrl||p.sourceUrl||p.officialUrl)}" target="_blank" rel="noopener">가격·메뉴 출처</a></div></div></article>`).join('');const featuredIds=new Set(['unafuji','shinshin','rakutenti','mentaiju']);document.querySelector('#legacy-food-list').innerHTML=places.filter(p=>p.category==='food'&&!featuredIds.has(p.id)).map(p=>`<article class="food-card"><span>${esc(p.type||'맛집 후보')}</span><h3>${esc(p.name)}</h3><p>${esc(p.caption||'대표 메뉴는 공식 메뉴에서 확인하세요.')}</p><p>${esc(p.tip||'방문 직전 영업 정보를 확인하세요.')}</p><div class="card-links"><a class="important-link" href="${googleMapsUrl(p)}" target="_blank" rel="noopener">Google Maps ↗</a>${p.officialUrl?`<a class="important-link" href="${esc(p.officialUrl)}" target="_blank" rel="noopener">공식 정보 ↗</a>`:''}</div></article>`).join('');}
...
function focusRestaurant(id,{scroll=false}={}){const place=restaurantGuide.find(item=>item.id===id);const marker=foodMarkers.get(id);if(!place)return;if(foodMap&&marker){foodMap.setView([place.lat,place.lng],Math.max(foodMap.getZoom(),16),{animate:!reducedMotion});marker.openTooltip();}const card=document.querySelector(`#restaurant-${CSS.escape(id)}`);document.querySelectorAll('.restaurant-card.is-highlighted').forEach(item=>item.classList.remove('is-highlighted'));card?.classList.add('is-highlighted');if(scroll){card?.scrollIntoView({behavior:reducedMotion?'auto':'smooth',block:'center'});card?.focus({preventScroll:true});const status=document.querySelector('#food-map-status');if(status)status.textContent=`${place.number}번 ${place.koreanName} 카드를 선택했습니다.`;}setTimeout(()=>card?.classList.remove('is-highlighted'),2200);}
function initFoodMap(){const status=document.querySelector('#food-map-status');if(foodMap){foodMap.invalidateSize();return;}if(!window.L){if(status)status.textContent='지도를 불러오지 못했어요. 각 카드의 Google Maps 링크를 이용해 주세요.';return;}foodMap=L.map('food-map',{zoomControl:true,scrollWheelZoom:false});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors'}).addTo(foodMap);const bounds=[];restaurantGuide.forEach(place=>{const icon=L.divIcon({className:'food-number-icon',html:`<span>${place.number}</span>`,iconSize:[30,30],iconAnchor:[15,15]});const marker=L.marker([place.lat,place.lng],{icon,title:`${place.number}. ${place.koreanName}`}).addTo(foodMap).bindTooltip(safeTooltipContent(document,`${place.number}. ${place.koreanName}`),{direction:'top',offset:[0,-12],className:'food-map-tooltip'}).on('click',()=>focusRestaurant(place.id,{scroll:true}));foodMarkers.set(place.id,marker);bounds.push([place.lat,place.lng]);});foodMap.fitBounds(bounds,{padding:[28,28],maxZoom:14});if(status)status.textContent='지도 번호를 누르면 같은 번호의 맛집 카드로 이동합니다. 지도 타일: © OpenStreetMap contributors';}
