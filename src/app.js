...
function renderSocialFood(){
  ...
  list.innerHTML=socialFoodCandidates.map(candidate=>{
    ...
    const mapping=numbers.length?`<p class="social-food-duplicate">정식 #${numbers.join('·#')} 카드에 반영 완료</p>`:'';
    ...
    return `<article class="social-food-card"><div class="social-food-photo"><img src="${esc(candidate.image)}" alt="${esc(candidate.title)} SNS 저장 사진" loading="lazy" width="527" height="695"></div><div class="social-food-copy"><span class="social-food-badge is-read">상호·지점 대조 완료</span><h3>${esc(candidate.title)}</h3><p>${esc(candidate.subtitle)}</p>${mapping}<div class="social-food-actions">${cardButton}<a class="social-map-link" href="${googleMapsUrl(candidate)}" target="_blank" rel="noopener">Google Maps ↗</a></div><small>사용자 제공 SNS 캡처 · 원 게시물/이용 권한 확인 권장</small></div></article>`;
  }).join('');
}
